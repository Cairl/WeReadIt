...
- **修复**: `exchanger.py` 查询奖励的 `_call_exchange` 调用增加 try/except，Token 过期时 re-raise，与单个奖励兑换循环行为保持一致。查询的网络异常不重试（与兑换循环不同），直接抛出由 `app.py` 兜底。
...
- **变更**: `constants.py` `PUSH_MAX_ATTEMPTS` 从 5 降到 3，减少推送重试次数避免长时间阻塞 GitHub Actions（退避间隔 180-360s 下 5 次过慢）。

### 2026-07-21

- **修复**: `app.py` 引入 `has_failure` 标志，Token 过期时推送以 `is_success=False` 调用，与 `exit_code=1` 状态一致（此前仍传 `is_success=True` 导致告警被掩盖）。
- **修复**: `config.py` 新增 `_env_renamed()` 辅助函数，三个推送渠道 Secret 加旧名 fallback（`PUSHPLUS_TOKEN` / `WXPUSHER_SPT` / `SERVERCHAN_SPT`），命中旧名时发 deprecated 警告，老用户升级后推送不再静默失效。
- **优化**: `reader.py` 恢复 synckey "连续第 X/Y 次" 计数日志，并在"synckey 修复成功"日志中带 `no_synckey_fix_triggered` 计数，便于排查熔断与统计修复成功率。
- **维护**: `.workbuddy/` 加入 `.gitignore`，移除已入库的 `.workbuddy/memory/*.md`（违反".workbuddy 只放临时计划文件"规则）。
- **测试**: 新增 `tests/test_app.py` 覆盖 Token 过期分支（`exit_code=1` + `is_success=False`）和其他兑换错误分支；`tests/test_exchanger.py` 补查询 re-raise 测试。
