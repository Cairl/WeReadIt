...
- **预热阶段(warmup)与日志格式统一**: 阅读前独立建立阅读上下文的 fix/重试隔离到 `_warmup`，不计入 120 次正式阅读；进度回显从裸 `print` 改为 `logger.info`（时间-级别-模块名 格式），消除 GitHub Action 日志中突兀的未格式化回显。`reader.py` 重构成 `_prepare_data` / `_read_once` / `_warmup` / `read_books`，保活逻辑（启动 refresh_cookie、3 种 COOKIE_DATA_VARIANTS、fix_no_synckey、last_time 偏移、data.pop("s")、30s sleep 等）全部保留。
- **ReadResult 预热字段**: `models.py` 新增 `warmup_done` / `warmup_attempts` 字段，`summary()` 附带「预热成功(尝试 N 次)」信息。
...
- **熔断计数器连续语义回归**: 重构 `read_books` 时漏写「成功即清零」逻辑，熔断从「连续 3 次」退化为「累计 3 次」，违背保活设计；已恢复 `no_synckey_streak` / `cookie_fail_streak` 在成功与 cookie 失效时清零（commit `6c4a9a0`），新增 `test_no_synckey_streak_resets_between_successes` 回归测试守护（临时回退清零即失败）。
- **reader 日志文案**: VIA_FIX 路径补打「已自动修复并重试」日志，与 `test_fix_log_merged_info_once` 断言一致。
...
- **app.py 改用 setup_logging()**: 移除裸 `print` 的 `refresh_print` 参数与 `make_refresh_print` 导入，`test_app.py` 同步去掉 6 处过时的 `make_refresh_print` patch，日志 handler 由 `setup_logging()` 统一安装；`test_keepalive.py` 适配预热阶段 mock 序列（35 测试全绿，保活断言未削弱）。
