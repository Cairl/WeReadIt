...
from config import data, headers, cookies, READ_NUM, PUSH_METHOD, book, chapter, WEREAD_ACCESS_TOKEN, EXCHANGE_AWARD, WEREAD_PLATFORM
...
def refresh_cookie():
    ...
    if new_skey:
        ...
        ERROR_CODE = "无法获取新密钥或者 WEREADIT_CURL_BASH 配置有误，终止运行。"
...
if WEREAD_ACCESS_TOKEN:
    ...
    try:
        ...
        if not vid:
            logging.warning("cookie 中未找到 wr_vid，跳过兑换")
            exchange_summary = "兑换奖励失败: cookie 中未找到 wr_vid"
        else:
            exchange_summary = exchange_awards(WEREAD_ACCESS_TOKEN, vid, EXCHANGE_AWARD, WEREAD_PLATFORM)
    except Exception as exc:
        logging.error("兑换奖励异常: %s", exc)
