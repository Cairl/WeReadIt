...
背景：旧实现把 /web/pay/balance 的 giftBalance 当作"赠币余额"在兑换前查询
推送，推送数值总比 App 滞后一次兑换、看似"不动"，且命中值用 mock 自证。
2026-07-30 用本模块对真实账号全量字段拍平易证：giftBalance/balance 实为
实时钱包余额（含赠币，随兑换/消费变动），体验卡以 memberCardSummary 为准，
赠币无独立实时总额接口。公开逆向文档未收录这些结论，本模块保留为诊断工具：
对候选接口发只读请求并拍平数值字段，供日后接口变动时复核，不猜、不编、
不拿 mock 值冒充真实命中。
