...
# 兑换阅读奖励时需填，APP端认证token，从APP抓包获取，未配置则跳过兑换
# Android平台填accessToken值，iOS平台填skey值
WEREAD_ACCESS_TOKEN = "" or os.getenv("WEREAD_ACCESS_TOKEN")
# 兑换平台，android或ios，决定用哪种认证方式和pf标识，默认android
WEREAD_PLATFORM = "android" or os.getenv("WEREAD_PLATFORM")
# 兑换策略，8位逗号分隔，0=不兑/1=体验卡/2=书币，对应奖励等级[4,5,1,2,3,11,12,13]
EXCHANGE_AWARD = "2,2,2,2,2,2,2,2" or os.getenv("EXCHANGE_AWARD")


# read接口的bash命令，本地部署时可对应替换headers、cookies
curl_str = os.getenv('WEREADIT_CURL_BASH')

# headers、cookies是一个省略模版，本地部署时对应替换
cookies = {
    'RK': 'oxEY1bTnXf',
